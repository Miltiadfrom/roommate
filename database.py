...
class Database:
    ...
    def authenticate_user(self, phone: str, password: str) -> Optional[User]:
        ...
        with self.get_connection() as conn:
            ...
            row = cursor.fetchone()
            if row and encryption_manager.verify_password(password, row['password_hash']):
        ...
        try:
            with self.get_connection() as conn:
                ...
                ''', (
                    ...
                    profile.gender,
                    profile.occupation,
                    encryption_manager.encrypt(profile.contact_info) if profile.contact_info else "",
                    profile.photo_path,
                    profile.budget_min,
                    profile.budget_max,
        ...
        with self.get_connection() as conn:
            ...
            if row:
                return Profile(
                    ...
                    contact_info=encryption_manager.decrypt(row['contact_info']) if row['contact_info'] else "",
    ...
    # === Свайпы ===
    def add_swipe(self, user_id: int, target_user_id: int, direction: str) -> bool:
        """Добавление свайпа"""
        try:
            with self.get_connection() as conn:
                cursor = conn.cursor()
                ...
                # Проверка на матч (если оба свайпнули вправо)
                if direction == 'right':
                    cursor.execute('''
                    ...
                    row = cursor.fetchone()
                    if row and row['direction'] == 'right':
                        # Создаем матч
                        cursor.execute('''
                            INSERT INTO matches (user1_id, user2_id, created_at)
                            VALUES (?, ?, ?)
                        ''', (min(user_id, target_user_id), max(user_id, target_user_id), datetime.now().isoformat()))
                        self._log_action(user_id, 'match', f'Match with user {target_user_id}')

                return True
        except Exception as e:
            print(f"Error adding swipe: {e}")
            return False

    def get_next_candidate(self, user_id: int) -> Optional[Profile]:
        """Получение следующего кандидата для свайпа"""
        with self.get_connection() as conn:
            cursor = conn.cursor()
            cursor.execute('''
    ...
    def send_message(self, sender_id: int, receiver_id: int, content: str) -> int:
        ...
        with self.get_connection() as conn:
            ...
            cursor.execute('''
                ...
                VALUES (?, ?, ?, ?)
            ''', (sender_id, receiver_id, encrypted_content, datetime.now().isoformat()))
            return cursor.lastrowid
        ...
        with self.get_connection() as conn:
            ...
            for row in cursor.fetchall():
                messages.append(Message(
                    ...
                    content=encryption_manager.decrypt(row['content']),
                    timestamp=row['timestamp'],
    ...
    def create_backup(self) -> str:
        ...
        self._log_action(0, 'backup', f'Backup created: {backup_path}')
        return backup_path
    ...
    def restore_backup(self, backup_path: str) -> bool:
        ...
        try:
            ...
            self._log_action(0, 'restore', f'Backup restored: {backup_path}')
            return True
        except Exception as e:
            print(f"Error restoring backup: {e}")
            return False

    def export_data(self, user_id: int) -> str:
        ...
        if profile:
            ...
            export_path = os.path.join(BACKUP_DIR, f'user_{user_id}_export.json')
            with open(export_path, 'w', encoding='utf-8') as f:
                json.dump(data, f, ensure_ascii=False, indent=2)
            return export_path
        return ""

    def import_data(self, user_id: int, json_path: str) -> bool:
